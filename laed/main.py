...
def train(model, train_feed, valid_feed, test_feed, config, evaluator, gen=None):
    ...
    while True:
        ...
        while True:
            ...
            ctx_utts = model.np2var(batch['contexts'], LONG)
            out_utts = model.np2var(batch['outputs'], LONG)
            torch.save((out_utts, ctx_utts), '/persist/git/research/will/flow-induction/laed_batches/' + str(batch_cnt) + '.torchsave')
